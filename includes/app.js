$("document").ready(function () {
    ...
    for(var i=0;i<=6;i++){
        $('th:nth-child('+(i+2)+')').text(moment(date).format('DD/MM')).css({"font-weight":"bold"}); /*add by meee */
        $('tr:nth-child(1) td:nth-child('+(i+2)+')').text(days[i]).css({"font-weight":"bold"});
        date.setDate(date.getDate() +1);
    }

    $(window).on("scroll touchmove", function () {
        $('header').toggleClass('tiny', $(document).scrollTop() > 0);  /*add by meee */
        $('a#logo').toggleClass('tiny', $(document).scrollTop() > 0); /*add by meee */
        $('header div').toggleClass('tiny', $(document).scrollTop() > 0); /*add by meee */
                                                                            /*add by meee */
        $('header span').toggleClass('tiny', $(document).scrollTop() > 0);       /*add by meee */
        $('header img').toggleClass('tiny', $(document).scrollTop() > 0);
        // $('.dropdown-menu').toggleClass('tiny', $(document).scrollTop() > 0);
        // $('.buttonBadge').toggleClass('tiny', $(document).scrollTop() > 0); /*add by meee */
    });
    ...
    // glyphicon-ok-sign ---> pencil
    // $('.glyphicon-ok-sign').siblings().eq(1).css({"display" :"none"});
    // $('.glyphicon-ok-sign').siblings().eq(0).css({"display" :"block"});
    ...
    $('.glyphicon-pencil').click(function () {
        ...
        // $(this).parent().parent().children().eq(0).removeClass("glyphicon-ok-sign").toggleClass("glyphicon-question-sign");
    ...
    $(".change").click(function () {
        $(".alert-success > p").text("Saved your manual changes");
        $(".alert-success").alert();
        $(".alert-success").fadeTo(2000, 500).slideUp(500, function () {
            $(".alert-success").slideUp(500);
        });
...
// Approved by manager change to status 3
...
// Automatic Algo
    // $("#automaticAlgo").bind('click',function(){
    //     week = $(this).data("week");
    //     user = $(this).data("user");
    //     console.log(week+"--"+user);
    //     // Here you can make validations about the limits of shifts
